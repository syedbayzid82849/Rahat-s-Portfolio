        ...
        <section id="contact" className="py-20 border-t border-gray-700">
            <div className="container mx-auto px-3 lg:px-6">
                ...
                <div className="grid lg:grid-cols-2 gap-16">
                        ...
                        <div className="glass rounded-3xl p-4">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                                    <Button type="submit" size="lg" className="w-full bg-gradient-primary hover:bg-gradient-primary/90  shadow-glow group flex justify-center items-center gap-2.5 p-3 rounded-xl">
                                        <span><Send size={16} className="group-hover:translate-x-1 transition-transform" /></span>
                    ...
                    <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} viewport={{ once: false }} className="space-y-8">
                        <div>
                            <h3 className="text-2xl font-bold mb-6 text-green-400">Get in Touch</h3>
                            <div className="space-y-4">
                                {contactInfo.map((info, index) => (
                                    <motion.div key={info.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.1 }} viewport={{ once: false }} className="flex items-center space-x-4 p-4 glass rounded-xl hover:shadow-glow transition-all duration-300">
                        ...
                        <div>
                            ...
                            <div className="flex space-x-4">
                                {socialLinks.map((social, index) => (
                                    <motion.a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} whileHover={{ scale: 1.1 }} transition={{ duration: 0.3, delay: index * 0.1 }} viewport={{ once: false }} className={`w-12 h-12 glass rounded-xl flex items-center justify-center transition-all duration-300 ${social.color} text-foreground`}>
                                        {social.icon}
                                    </motion.a>
                                ))}
                            </div>
